...
6. Persist or visualize only when the user requests it; treat the emitted CSV/JSON schema as the import contract.
...
When Neo4j persistence is requested, derive constraints and import Cypher from the emitted CSV headers
and verify row counts after import; this bundled reference does not depend on a separate Neo4j runbook.
