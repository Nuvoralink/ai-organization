...
Apply those exact metadata predicates to the raw JSON and retain both filtered and unfiltered output.
...
Follow this bundled five-step workflow:

| Step | Action | Gate | Key Reference |
|------|--------|------|---------------|
| 1 | Resolve output dir, detect languages + Pro availability | — | Use Glob, not Bash |
| 2 | Select scan mode + explicit installed rulesets | — | Record exact configs |
| 3 | Present plan, get explicit approval | ⛔ HARD | AskUserQuestion |
| 4 | Spawn bounded parallel scan tasks | — | Each task receives target, config, output, and exit contract |
...
**Merge rule (Step 5):** use an existing repository-approved SARIF merger only after verifying its own
tests and exit. If none exists, retain each valid per-scan SARIF and report that no merged artifact was
produced; never invent or assume a bundled merge script.
...
This bundled guide is self-contained. Resolve available rulesets from the installed Semgrep version and
approved local configuration; never invent an absent catalogue, task template, workflow, or merge tool.
Each scanner brief must name the absolute target, exact configs/mode, metrics-off requirement, output
files, raw exit, and no-write boundary.
