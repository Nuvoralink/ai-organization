...
- Use the core workflow below plus the target repository's authoritative platform runbooks.
<!-- dual-compat-end -->
...
The Vault lifecycle and emergency-revocation controls in this document are the bundled runbook floor;
use the deployment's authoritative Vault operations manual for environment-specific commands.
...
Map each gate to the project's authoritative framework/control register; do not infer compliance from
a scanner result or an unbundled mapping.
...
The runtime checklist below is the bundled implementation floor.
...
For deployment-specific HA, DR, engines, AppRole, dynamic credentials, and rotation, use the verified
runbook owned by the environment; this reference does not invent missing operational commands.
...
Exercise AWS/Transit/PKI/Kubernetes/JWT/Vault-Agent examples only from the environment's tested Vault
runbook and capture a non-production verification; no unbundled examples are assumed here.
...
Use the installed Falco version's primary documentation for commands, then retain the effective rules
and a triggered non-production alert as proof.
...
Retain the actual installed ConstraintTemplate/Constraint and one admitted/denied fixture; do not rely
on an unbundled manifest.
...
Use the repository's installed scanner/attestation workflow and verify SARIF, SBOM digest binding, and
every ignore row directly.

## Reference boundary

This bundled guide is self-contained. Environment-specific Vault, compliance, container-runtime,
automation, and pipeline-governance runbooks must be read from the target repository or platform's
authoritative documentation and verified live; absent companion files are never assumed.
