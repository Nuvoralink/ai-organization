...
**Resource boundary:** this bundled document is self-contained; use installed CodeQL primary docs for
version-specific syntax and do not assume auxiliary templates/workflows exist.
...
- **"The build fails on macOS, just use build-mode=none"** - Diagnose architecture/toolchain mismatch and try a matching toolchain or Rosetta before accepting an incomplete database.
...
| Workflow | Purpose |
|----------|---------|
| Build database | Create a CodeQL database using build methods in sequence |
| Create data extensions | Detect or generate models only for verified project APIs |
| Run analysis | Select explicit suites, execute queries, and inspect raw SARIF |
...
This bundled guide is self-contained. Use the installed CodeQL version's primary documentation for
platform-specific build tracing and query-pack syntax. Retain exact database metadata, explicit suites,
raw SARIF, and command exits so a missing auxiliary template cannot silently narrow coverage.
