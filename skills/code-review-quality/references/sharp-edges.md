...
A separate reviewer may run this four-phase workflow, but delegation is optional and this file is the
self-contained authority. Verify the delegated findings against the actual diff and outputs.
...
Reject unvalidated constructor parameters directly under the configuration-cliff criteria above.
...
## Self-contained language application

Apply the same misuse tests in every language: probe zero/empty/null/negative inputs; replace stringly
security values with semantic types or registries; make secure defaults the only easy path; require
errors to fail closed; and test parameter swaps, ignored return values, timeout boundaries, and unsafe
configuration combinations. This bundled reference does not depend on separate language guides.
...
- [ ] Constructor params validated, not merely given safe defaults
