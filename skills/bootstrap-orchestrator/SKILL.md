...
1. **Vendor-neutral governor + adapters.** Install `core/lifecycle/task-governor.mjs` and the task/evidence schemas from the universal control-plane repository. Wire Claude's `TaskCreated`/`TaskCompleted` adapter and the project's Codex completion command to call the same governor; neither adapter may reduce proof or path checks. Then install `.claude/settings.json` from `templates/settings.json.template` plus `scripts/claude-lifecycle-hook.mjs` from `templates/lifecycle/claude-lifecycle-hook.mjs.template` — the read-only allowlist is built from the repo's REAL check/test commands (discovered in Step 1), plus lifecycle hooks and the `PostToolUse (Edit|Write)` gate router. Fill the integration branch and one bounded completion gate, ignore `tmp/agent-telemetry/`, instantiate `templates/tests/claude-lifecycle-hook.test.ts.template` in the repo's real test lane, and prove the hook with temp-tree mutation tests. The lifecycle command emits live local state at SessionStart/SubagentStart without converting command failure into a false clean/zero claim; validates a structured task assurance contract; fingerprints read-only task-time state; makes changed-file implementation checks bite at TaskCompleted; requires risk-selected proofs with opened artifacts and killer mutations; requires substantive Doctrine-loop/Honesty report sections at SubagentStop; and records only hashed IDs, allowlisted counts/outcomes, summary hash/bytes, and duration under ignored `tmp/agent-telemetry`. Do not persist prompt/report/summary/path bodies, do not allow a telemetry-path override, and do not configure `WorktreeCreate` until the installed Claude version's replacement semantics and Windows path behavior are explicitly verified.
...
4. Plan/apply/verify are separate: dry-run is no-write; apply is atomic and refuses dirty managed targets; verify fails on changed/missing/local-only managed files, unsafe imports, unknown roles, stale locks, or a retired path returning. Line-ending-only changes are normalized. Keep the portable committed ownership/hash manifest distinct from the runtime install lock and rollback snapshots. Runtime locks/snapshots live under the registered user profile, store tokenized destinations, and never enter the product repository with absolute machine paths, timestamps, or backup history.
