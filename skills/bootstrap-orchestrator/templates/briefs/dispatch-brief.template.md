...
<!-- For a bounded CLAUDE CLI dispatch on Windows, use the canonical user-owned helper:
     ...
     Supply this brief as --prompt-file; exact built-in names as --tools; a safe --mode; and explicit stdout/stderr paths. Prefer Agent for new delegation-capable dispatches; legacy Task is treated as the same exact tool only because Claude renamed it.
     If --tools contains anything outside Read,Glob,Grep,WebFetch,WebSearch, also supply --handoff-ref with the GitHub issue/PR artifact. The dispatcher itself must successfully materialize that live artifact before Claude starts; a local prompt or pasted snapshot is not a substitute.
     `dontAsk` cannot perform Edit/Write. User-authorized bounded non-interactive implementation uses `bypassPermissions` only with the exact boundary marker above, the tool subset Read,Glob,Grep,Edit,Write plus optional Skill, and no irreversible/billed/external authority. If Skill is enabled, skill_names must list each exact trusted local skill; Skill loads context but cannot add Agent/Task, shell, or any tool absent from the parent allowlist. Caller-authored prompt text must contain no `@` character because Claude resolves prompt-side @file references without a Read tool event; spell mentions/addresses without `@` and declare every readable file/directory in read_paths.
     The explicit read-only diagnostic set above is the counterexample and does not require a handoff reference.
     A provided PR handoff is materialized even for a Read/Glob/Grep-only audit: cwd must be the clean repository root, origin must match, checked-out HEAD must equal the live PR head, and the live base/head OIDs plus their verified merge-base must exist locally. Full PR proof always includes changed-file count/path digest plus raw full-index blob/mode/status digest. The appended `--binary` patch is full for an under-bound review, exact edit_paths for implementation, or one explicit partial-review scope after an oversized full patch. Issue handoffs require no diff.
