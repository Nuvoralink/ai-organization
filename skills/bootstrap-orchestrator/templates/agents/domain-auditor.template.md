...
Implementers running the start-of-work / pre-report self-audit read THIS block, not the audit checklist above (auditor-seat boundaries are not yours). An implementer's "domain lens run, clean" claim is a lead — it never narrows this lens's scope. Design these in, then verify on your own diff: every acting/deciding behavior classified against the project's authority tiers BEFORE building (autonomy where the doctrine reserves the decision is a STOP); a gate/policy/validator you change ADMITS only the intended set AND still REJECTS the case just outside it — with the NEGATIVE test written (widen the predicate further → it must go RED); nothing fails open that must fail closed; disabled/degraded paths write honest audit evidence, never a fabricated pass.
