#!/usr/bin/env node
// Canonical Claude lifecycle router instantiated for the Voice Agent Platform. Keep
...
function missingSections(text, requirements) {
  ...
  return requirements
    .filter((requirement) => {
        ...
        (line, index) =>
          ...
          (requirements.some((candidate) => sectionLine(line, candidate.aliases)) ||
            ...
            (startHeadingLevel !== undefined &&
              (line.match(/^\s*(#{1,6})\s+/)?.[1].length ?? Number.POSITIVE_INFINITY) <=
                startHeadingLevel)),
      );
      const heading = lines[start];
      const body = lines.slice(start + 1, end < 0 ? lines.length : end);
      const inline = heading.includes(':')
        ? heading
            .slice(heading.indexOf(':') + 1)
            .replaceAll('*', '')
            .trim()
        : '';
      const substantive =
      ...
      return (
        Boolean(requirement.numbered) && !body.some((line) => /^\s*\d{1,2}[.)]\s+\S/.test(line))
      );
    })
...
function implementationDeliveryFitFailure(text) {
  ...
  const fits = lines.flatMap((line, index) => {
    const match = line.match(
      /^\s*(?:[-*+]\s+)?(?:\*\*)?Delivery fit(?:\*\*)?\s*:\s*(single-turn|checkpointed)\b/i,
    );
  ...
  const nextHeading = lines.findIndex(
    (line, index) => index > fitIndex && /^\s*#{1,6}\s+/.test(line),
  );
  ...
  if (mode === 'single-turn') {
    const estimate = block
      .map(
        (line) =>
          line.match(/^\s*(?:[-*+]\s+)?(?:\*\*)?Estimated scope(?:\*\*)?\s*:\s*(.+)$/i)?.[1],
      )
      .find((value) => value !== undefined);
    const coherence = block
      .map((line) => line.match(/^\s*(?:[-*+]\s+)?(?:\*\*)?Coherence(?:\*\*)?\s*:\s*(.+)$/i)?.[1])
      .find((value) => value !== undefined);
    const hasEstimate = substantive(estimate);
    const hasCoherence = substantive(coherence);
    return hasEstimate && hasCoherence
      ? undefined
      : 'Delivery fit (single-turn needs Estimated scope and Coherence)';
  }
  ...
  const ordered =
    ...
    checkpoints.every((checkpoint, index) => checkpoint.number === index + 1);
  ...
  return ordered && safe
    ? undefined
    : 'Delivery fit (checkpointed needs ordered checkpoints with authority state and verification)';
}
...
function emitContext(eventName, content) {
  process.stdout.write(
    `${JSON.stringify({ hookSpecificOutput: { hookEventName: eventName, additionalContext: content } })}\n`,
  );
}
...
function profileRegistry(root) {
  return JSON.parse(
    readFileSync(path.join(root, '.ai-organization', 'proof-profiles.json'), 'utf8'),
  );
}
...
function stateSummary(root) {
  ...
  const branchAvailable =
    branchResult.status === 0 && !branchResult.error && branchResult.stdout.trim();
  ...
  const dirty =
    dirtyResult.status === 0 && !dirtyResult.error
  ...
  const worktrees =
    worktreeResult.status === 0 && !worktreeResult.error
      ? String(
...
function runCompletionGate(root) {
  ...
  const args =
    process.platform === 'win32'
      ? ['/d', '/s', '/c', 'npm', 'run', TASK_COMPLETION_GATE]
      : ['run', TASK_COMPLETION_GATE];
...
async function handle(payload, root) {
  ...
  switch (payload?.hook_event_name) {
    ...
    case 'SubagentStart':
      ...
      emitContext(
        'SubagentStart',
        `Use the six-part brief, stay in the assigned worktree, read command-owned exits, and report substantive Doctrine-loop findings plus Honesty clause sections.\n${stateSummary(root)}`,
      );
      return;
    case 'TaskCreated': {
      const description =
        typeof payload.task_description === 'string' ? payload.task_description : '';
      ...
      if (missing.length) {
        ...
        block(
          `[lifecycle] TaskCreated blocked. Add explicit, substantive sections for: ${missing.join(', ')}. Procedure needs a numbered step; implementation work also needs a valid Delivery fit declaration.`,
        );
        return;
      }
    ...
    case 'TaskCompleted': {
      ...
      if (!accepted)
        return block('[lifecycle] TaskCompleted blocked: no accepted TaskCreated state.');
      if (
        payload.completion_report ||
        structuredMarker(payload.task_description, 'COMPLETION_REPORT_JSON:')
      ) {
        return block(
          '[lifecycle] TaskCompleted blocked: the official event has no completion-report field; use a platform SubagentStop report receipt.',
        );
      }
      let changed = { files: [] };
      ...
      if (accepted.state !== 'completed' && accepted.completion_mode === 'implementation') {
        ...
        preflightFailures =
          diffChecks.filter((result) => result.status !== 0 || result.error).length +
          (gate.status === 0 && !gate.error ? 0 : 1);
      }
      if (preflightFailures) {
        recordEvent(root, payload, {
          outcome: 'block',
          completionTier: accepted.completion_mode,
          changedFileCount: changed.files.length,
          gateCount: 1,
          failedCheckCount: preflightFailures,
        });
        block(
          `[lifecycle] TaskCompleted blocked: changed-file completion checks failed.\n${gateOutput}`,
        );
        return;
      }
      ...
      recordEvent(root, payload, {
        ...
        completionTier: accepted.completion_mode,
        changedFileCount: changed.files.length,
        gateCount: accepted.completion_mode === 'implementation' ? 1 : 0,
        failedCheckCount: result.failures.length,
      });
      if (result.accepted) await safelyReleaseTaskClaim(payload, root);
      if (!result.accepted)
        block(`[lifecycle] TaskCompleted blocked: ${result.failures.join('; ')}`);
      return;
    }
    case 'SubagentStop': {
      const report =
      ...
      recordEvent(root, payload, {
        outcome: failures.length ? 'block' : 'allow',
        missingSectionCount: failures.length,
      });
      ...
      if (failures.length)
        block(
          `[lifecycle] SubagentStop blocked. ${receiptFailures.join('; ')} Add substantive sections for: ${missing.join(', ')}.`,
        );
      return;
    }
    case 'PostCompact': {
    ...
    case 'SessionEnd': {
      ...
      recordEvent(root, payload, {
        durationMs: started ? Math.max(0, Date.now() - started) : null,
      });
      return;
    }
    default:
...
if (!root) {
  if (BLOCKING_EVENTS.has(payload?.hook_event_name))
    block(`[lifecycle] ${payload.hook_event_name} blocked: git worktree unavailable.`);
  process.exit();
}
try {
  await handle(payload, root);
} catch {
  if (BLOCKING_EVENTS.has(payload?.hook_event_name))
    block(
      `[lifecycle] ${payload.hook_event_name} blocked: lifecycle contract could not be evaluated safely.`,
    );
}
