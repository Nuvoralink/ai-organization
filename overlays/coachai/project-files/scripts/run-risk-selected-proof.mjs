...
function changedFiles(root) {
  const base = git(root, ['merge-base', 'origin/main', 'HEAD']) || git(root, ['merge-base', 'main', 'HEAD']);
  const lists = [base ? git(root, ['diff', '--name-only', `${base}...HEAD`]) : '', git(root, ['diff', '--name-only']), git(root, ['diff', '--cached', '--name-only']), git(root, ['ls-files', '--others', '--exclude-standard'])];
...
if (import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  const files = explicit.length ? explicit : changedFiles(process.cwd());
  if (files.length === 0) { console.log('risk-proof: PASS — no changed paths'); process.exit(0); }
