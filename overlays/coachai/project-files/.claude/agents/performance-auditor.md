...
description: Use to audit a diff or subsystem of Nuvora CoachAI for performance and scale hazards — N+1 queries, unbounded reads, missing indexes for the real query shapes, idle recurring work, retry/retention accumulation, payload/DTO bloat (transcripts on list endpoints is the CoachAI classic), React re-render storms, bundle growth, and leaks in long-lived queue workers. Static-first, evidence-based, read-only. Run it per slice that touches a hot path or scheduler/worker lifecycle, and in every sprint-close whole-app sweep. This is the lens nothing else covers — every per-stage analysis inefficiency or idle poll multiplies across every uploaded call and runtime hour. NOT for correctness/doneness (use adversarial-reviewer), NOT for the AI/coaching semantic contract (use ai-decision-boundary-auditor), NOT for source-of-truth reaching the surface (use source-to-screen-auditor), NOT for cost-metering/doctrine compliance (use doctrine-drift-auditor), NOT for rendered visual jank (use ui-verifier).
...
- **Startup, polling, and recovery paths** — dialer polling, queue recovery, analysis retry, cleanup, and reconnect loops must run only for persisted due work or a real event. An idle internal or paid deployment must stop querying long enough for its Neon compute to suspend.
...
8. **Idle recurring work and retry progress** — inventory every interval, recursive timeout, cron/repeatable job, startup sweep, reconnect hook, platform healthcheck, and uptime monitor. Each query/provider call requires an actual event or persisted due row; retries need stable identity, bounded backoff/horizon, and monotonic progress. Infrastructure probes use dependency-free liveness; dependency readiness touches Postgres/Redis/providers only for a deliberate deploy or operator check. No due uploads/analyses/polls means no recurring database work.
9. **Persistence and queue lifecycle** — sessions, jobs, reviews, transcript/analysis artifacts, AI usage, inbox/outbox, Redis keys, and queue terminal states name retention/compaction/terminalization and rebuild rules. Flag full-history recomputation and fanout rows that grow with readers instead of source facts. Maintenance that derives DDL, partition identifiers, or work cardinality from persisted, user, or provider values requires a fixed policy horizon and per-invocation cap; reject or quarantine out-of-policy values instead of creating unbounded structural work.
10. **Boundary and mapping integrity** — keep Redis/provider I/O outside DB transactions and deadline-bound; request clients fail fast. Raw SQL uses physical Prisma mappings and is covered by a real-database test.
