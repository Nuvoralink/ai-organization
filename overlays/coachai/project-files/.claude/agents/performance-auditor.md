...
8. **Idle recurring work and retry progress** — inventory every interval, recursive timeout, cron/repeatable job, startup sweep, and reconnect hook. Each query/provider call requires an actual event or persisted due row; retries need stable identity, bounded backoff/horizon, and monotonic progress. No due uploads/analyses/polls means no recurring database work.
