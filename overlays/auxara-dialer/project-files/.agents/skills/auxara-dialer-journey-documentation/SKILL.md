---
name: auxara-dialer-journey-documentation
description: Update Auxara Dialer's AI-build journey document when a task reveals reusable lessons about prompting, source-of-truth design, AI-vs-deterministic boundaries, telephony-engine invariants, compliance gating, validation, testing, blast radius, implementation review, or the user's own interventions. Use after meaningful bugs, architecture corrections, prompt-quality discoveries, testing breakthroughs, or repeated workflow patterns; do not use for trivial one-off edits.
---

# Auxara Dialer Journey Documentation

## Purpose

Use this skill to keep `docs/Journey/AI_BUILD_JOURNEY_LESSONS.md` current as a founder/dev diary source — for TikTok, a course, a book, build-in-public notes, and internal "this is how we build" principles.

Do this *after* the actual work is handled. Do not let documentation replace the fix.

If a Journey update reveals a durable prevention rule that changes how future work should be done, update the living doc, agent rule, or skill that owns that behavior in the same pass. Do not let the Journey doc become a second hidden architecture spec.
...
- deterministic code pretending to understand semantic sales/coaching meaning (anti-pattern),
- AI code making decisions that should be deterministic (compliance gating, lifecycle transitions, billing) — also an anti-pattern,
...
- telephony-specific gotchas (Telnyx event ordering, AMD edge cases, number-reputation surprises, 10DLC vetting variance, dead-air at scale),
- compliance lessons (TCPA/CASL nuance, state DNC differences, recording-disclosure edge cases),
- useful testing patterns (golden call fixtures, dial-mode regression, source-to-screen smoke, replay of persisted Telnyx events),
...
Skip or only mention briefly when the situation is merely another instance of an already documented lesson.
...
For a pattern or section update, capture:

- what happened in plain language,
- what the visible bug looked like (booker softphone froze? wallboard wrong? wrong tenant saw data? compliance gate skipped?),
- what the shallow fix would have been,
- what the root cause was,
- whether the cause involved prompting, docs, architecture, source truth, validation, UI truth reconstruction, deterministic semantic logic, missing telephony invariant, missing compliance gate, blast radius, or user-side choices,
...
```text
I'm documenting my journey learning AI and building this app (Auxara Dialer — Telnyx-based power dialer). I want this to become useful for a TikTok series, course, book, or founder/dev diary.
...
- What happened in plain language.
- What the visible bug looked like (booker / manager / tenant admin / compliance auditor view).
- What the first/shallow fix would have been.
- What the real root cause was.
- Whether this was caused by unclear prompting, missing documentation, weak architecture, bad source-of-truth design, missing validation, UI reconstructing truth, deterministic code pretending to understand meaning, AI deciding something that should be deterministic, missing telephony invariant, missing compliance gate, incomplete blast-radius thinking, or something I asked/implemented/accepted that changed the outcome.
...
- Say when AI was wired into something that should have been deterministic (compliance, billing, lifecycle) — that's a more dangerous mistake in a regulated telephony product.
...
- Include useful tests when they taught a reusable lesson (synthetic golden call fixtures, replay of persisted Telnyx events, end-to-end source-to-UI tests).
...
- Keep the document organized and readable.
