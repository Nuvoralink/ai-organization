...
**Self-check is fast + bounded; the orchestrator owns the heavy/DB/local-CI gates.** The implementer runs only `typecheck` + `lint` + `format:check` + `gates:all` (cheap, small output) and **writes** the full test ladder — but does **not** run `npm run test:integration`, Docker builds, or the full `npm run verify`. The orchestrator runs the local CI-equivalent bundle via the `test-runner` agent (`npm run gate:audit` + `npm run verify` + `npm run test:integration` + `docker build -t auxara-backend:ci .`, serialized as the sole local test-DB user) — a disposable context that absorbs the verbose log and returns a compact pass/fail verdict — then adversarial review before merging. Rationale (2026-06-14, S1.2-C2 + 2026-07-11 S1.3 integration closure): the DB-backed suite is the single biggest cause of implementer hangs — Docker spin-up + contention on the one shared `auxara-testdb` (concurrent runners get phantom "DB unreachable" failures) + a verbose log flooding the agent's context at the end of an already-long run — and GitHub Actions can fail before any repo step executes. Ordinary PRs carry local evidence; remote CI is reserved for main/milestone/release/manual runs. Type errors and gate violations still get caught early (fast typecheck/gates); runtime/DB/container behavior is proven by the orchestrator locally. Codified in `.claude/agents/sprint-implementer.md` + `.claude/agents/test-runner.md` + `.claude/rules/sprint-rigor.md` §12.
