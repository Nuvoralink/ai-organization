...
- **Delivers functionality-first, with local verification as the proof authority.** Prove the intended code behavior (targeted biting tests → deploy safety → deployed journey) before broad assurance; sprint-close, integration, backend-contract, and release-candidate CLOSURE runs one `npm run ci`. GitHub-hosted CI is intentionally retired to avoid paid remote runners. See `.claude/rules/functionality-first-delivery.md`.
...
**Self-check is focused + bounded; the orchestrator owns one final heavy/DB/local-CI gate.** The implementer runs its exact biting tests plus `npm run proof:changed`, which selects and reports the completion profiles for the real diff. It does **not** run `gates:all`, `verify`, `test:integration`, `ci`, Docker, or `test:db:recover` inside the repair loop. For functional work the orchestrator first proves deploy safety and (with human production authority) deploys for **deployed functional proof** of the original journey; after acceptance and queued-finding remediation it fetches/rebases onto current main, runs focused fallout proof, freezes a clean candidate, and runs `npm run ci` once via the `test-runner` as the sprint/release closure gate. The runner refuses a dirty or stale-base candidate and uses a realistic single-command time budget, so a too-short wrapper cannot manufacture an expensive rerun. A failed CI returns to focused repair; it is redispatched only after the candidate is stable again. The shared Postgres/Redis bootstrap atomically leases Git's common directory and labels both fixed-name containers; a competing worktree receives `TEST_DB_LEASE_HELD` before any container mutation. Only the orchestrator may invoke explicit stale recovery after proving the owner is gone. GitHub-hosted CI is retired; runtime/DB/container behavior is proven locally. Codified in `.claude/agents/sprint-implementer.md`, `.claude/agents/test-runner.md`, and `.claude/rules/sprint-rigor.md` §12.
...
1. **Branch + worktree:** `git fetch origin && git worktree add -b <area>/<slice> <folder> origin/main` — **ALWAYS fetch immediately before cutting the worktree.** `origin/main` is a *local cached ref* that goes stale the moment a PR merges server-side; a worktree cut without fetching bases the whole slice on old code and earns a needless rebase or, worse, re-implements something already merged. After cutting, sanity-check the base: `git log --oneline -1` should show the commit you expect from the latest merged PR. (Bit us 2026-06-11: a worktree spawned one merged-PR behind and had to be hard-reset.) **`<folder>` is DERIVED from the current repo location — a sibling of this checkout (a `<repo>-worktrees/<slice>` directory next to the repo), never a path copied out of a dated brief.** The `docs/HANDOFF-*` and `docs/agent-prompts/sprint-1-*` briefs contain historical laptop paths (a different drive/root than this machine uses) — those are records of past runs, NOT live locations; copying a concrete worktree path from one lands the worktree on a stale/absent root. Derive the base from where the repo actually is (`git rev-parse --show-toplevel`), don't transcribe it.
2. **Work + commit on the branch** (never on `main`).
3. **Push the branch → open a PR → attach the exact local proof → merge it** (`gh pr merge --rebase --delete-branch` keeps history linear and deletes the branch). A functional feedback deploy merges on targeted proof + deploy safety + human authority (`.claude/rules/functionality-first-delivery.md`); sprint/release closure carries the orchestrator/test-runner `npm run ci` result as its evidence. GitHub-hosted CI is retired and no PR status substitutes for local output. The merge lands on `origin/main` server-side; no local `main` checkout is needed.
