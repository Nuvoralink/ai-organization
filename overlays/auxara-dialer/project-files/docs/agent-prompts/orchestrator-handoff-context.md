...
- **Uses local verification as the merge authority.** Every implementation PR carries the exact relevant local proof; sprint-close, integration, backend-contract, and release-candidate work runs `npm run ci`. GitHub-hosted CI is intentionally retired to avoid paid remote runners.
...
**Self-check is fast + bounded; the orchestrator owns the heavy/DB/local-CI gates.** The implementer runs only `typecheck` + `lint` + `format:check` + `gates:all` (cheap, small output) and **writes** the full test ladder — but does **not** run `npm run test:integration`, Docker builds, the full `npm run verify`, or `npm run test:db:recover`. The orchestrator runs `npm run ci` via the `test-runner` agent, serialized as the sole local test-DB user; that disposable context absorbs verbose output and returns a compact verdict before adversarial review. The shared Postgres/Redis bootstrap atomically leases Git's common directory and labels both fixed-name containers; a competing worktree receives `TEST_DB_LEASE_HELD` with owner evidence before any `docker rm`. No time/PID auto-steal exists. Only the orchestrator may invoke explicit stale recovery after proving the owner is gone. GitHub-hosted CI is retired; runtime/DB/container behavior is proven locally. Codified in `.claude/agents/sprint-implementer.md`, `.claude/agents/test-runner.md`, and `.claude/rules/sprint-rigor.md` §12.
