...
**Every dispatch brief for an agent that will take a value from a shared NAMESPACE reserves it FIRST.** Parallel/swarm agents in isolated worktrees each independently take "the next value" from a shared namespace — the next migration number, ADR number, decision-log ID, or a free dev port — and collide (git-clean until integration, or EADDRINUSE at runtime; the 0083×2 class). The brief instructs the agent to run the reservation CLI BEFORE creating the artifact and to use exactly the printed value:
- migration: `node scripts/reserve.mjs migration <slug>` (or `npm run db:reserve-migration -- <slug>`) → `NNNN_<slug>`, create `backend/prisma/migrations/NNNN_<slug>/`.
- ADR: `node scripts/reserve.mjs adr <DOMAIN> <kebab-title>` → `ADR-<DOMAIN>-NNN`.
- decision-log ID: `node scripts/reserve.mjs decision <PREFIX> <label>` → `<PREFIX>-NNN`.
- dev/test port: `node scripts/reserve.mjs port <purpose>` → a free port from the agent range.

The claim is atomic against a Git-common-dir ledger every worktree sees, so no two agents get the same value. Reactive BACKSTOPS catch any residual collision regardless of discipline: `gate:migration-numbering` + `gate:migration-object-names` (migration number + DB object name), `gate:adr-numbering` (ADR number), git (decision-log dup IDs, one file), and the OS (EADDRINUSE, ports). The orchestrator runs `node scripts/reserve.mjs --reconcile` when values land on `main` or branches are abandoned.

The mechanism is a MANAGED control-plane asset shared by every project: the project-agnostic core, the shared CLI, and both collision-gate engines live in `.ai-organization/runtime/core/coordination/` (canonical source: `core/coordination/` in the AI-Organization repo). Never edit a delivered copy — that forks the digest-pinned asset and the next install reverts it; fix it at the source and re-install. This repo ships only its own bindings in `scripts/reservation-config.mjs` plus the thin entry scripts that hand those bindings to the shared runners.
