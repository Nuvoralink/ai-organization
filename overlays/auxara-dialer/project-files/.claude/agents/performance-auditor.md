...
8. **Idle recurring work and retry progress** — inventory every interval, recursive timeout, BullMQ repeatable, startup/reconnect sweep, provider poll, platform healthcheck, and uptime monitor. Each database/provider action requires an actual event or persisted due row; retries need stable identity, bounded backoff/horizon, and monotonic progress. Infrastructure probes use dependency-free liveness; dependency readiness touches Postgres/Redis/providers only for a deliberate deploy or operator check. No active dial/provider work means no recurring database load.
9. **Persistence and queue lifecycle** — calls/events, appointments, callbacks, dispositions, stats/snapshots, webhook receipts, audit rows, Redis keys, and completed/failed jobs name retention/compaction/terminalization and rebuild rules. Flag per-reader fanout, queue-only truth, and full-history recomputation on routine reads. Maintenance that derives DDL, partition identifiers, or work cardinality from persisted, user, or provider values requires a fixed policy horizon and per-invocation cap; reject or quarantine out-of-policy values instead of creating unbounded structural work.
