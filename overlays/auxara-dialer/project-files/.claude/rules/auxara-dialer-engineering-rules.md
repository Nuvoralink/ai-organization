...
- **Verification follows the functionality-first lifecycle.** Inside implementation and repair, run exact biting tests plus `npm run proof:changed`; do not repeatedly invoke `verify`, `gates:all`, `test:integration`, or `ci`. A functional candidate first proves deploy safety (migration/schema application, DB integrity, build/startup, Railway readiness, irreversible-data protection), then—under explicit human production authority—merges/deploys promptly so the original user journey can supply **deployed functional proof**. Auditors may run in parallel, but ordinary findings queue without remediation until that proof passes. After acceptance, remediate queued findings, fetch/rebase current `main`, and invoke **`npm run ci` once as the complete sprint/release closure gate** (`gate:audit` → `verify:merge` → `test:integration` → `docker build`). Smaller checks never impersonate closure proof; broad closure proof never impersonates functional proof. GitHub-hosted CI remains retired.
