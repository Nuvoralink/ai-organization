...
- **Verification follows the functionality-first lifecycle.** Inside implementation and repair, run exact biting tests plus `npm run proof:changed`; do not repeatedly invoke `verify`, `gates:all`, `test:integration`, or `ci`. Applicable independent auditors are required before merge and may run in parallel; classify every finding, fix BLOCK now, and queue only verified bounded fail-safe FIX-NEXT residuals outside every blocker class with durable backlog rows. A zero-BLOCK functional candidate proves deploy safety, then—under explicit human production authority—merges/deploys promptly so the original journey supplies **deployed functional proof**. After acceptance, remediate queued findings, fetch/rebase current `main`, and invoke **`npm run ci` once as the complete sprint/release closure gate** (`gate:audit` → `verify:merge` → `test:integration` → `docker build`). Functionality-first changes remediation order, never auditor cadence. Smaller checks never impersonate closure proof; broad closure proof never impersonates functional proof. GitHub-hosted CI remains retired.
- **`gates:all` order has one authority.** `scripts/run-gates-all.mjs` owns the ordered registry, executable aggregate parser, and fail-fast runner; `scripts/run-gates-all.test.mjs` owns the immutable pre-registry 26-step baseline. Add a new root `gate:*`/`check:*` command by adding one registry row; do not append another `&&` chain to `package.json` or restate the inventory in prose. A relevant script must be registered directly or covered by a registered aggregate. Exactly `gate:audit` with its live `npm audit --omit=dev --audit-level=high` command is excluded because it remains sprint-close/integration-only; no other exclusion is valid.
