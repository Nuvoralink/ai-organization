...
- **Local verification is the merge authority.** For ordinary implementation work, run `npm run verify` before commit/integration (build + lint + format:check + typecheck + test + gates:all). `npm run gates:all` ALONE is NOT enough: it omits lint / format:check / typecheck. For sprint-close, integration-branch, backend-contract, or release-candidate work, run **`npm run ci` — the COMPLETE local merge gate** (`gate:audit` → `verify` → `test:integration` → `docker build`, cheapest signal first, fail-fast, true exit code propagated). Do not hand-assemble those four; hand-assembly is how the DB lane got skipped (DB-LANE-INVISIBLE-COVERAGE-001). The `ci` command exposes no lane-skip option; smaller checks use their own named commands and never impersonate merge proof. `gate:local-ci-contract` (in `gates:all`) fails if the local aggregate stops containing any required lane. GitHub-hosted CI is intentionally retired to avoid paid remote-runner execution; PR status checks are not proof and must not replace the exact local command output.
