...
- **Apply the centralized action boundary — this is what makes autonomous iteration safe.** `.ai-organization/policies/action-authority.v1.json` is authoritative. Branch, commit, push, and pull-request actions are allowed inside the authorized task. Merge is allowed only when every conditional-merge criterion is independently proven; otherwise it stops for a human. Production mutation/deploy/config/migration, destructive, billed, external-message/contact, secrets, and product/design/material-architecture actions always stop for a human. No critic's unverified "clean" can satisfy an independent-verification condition.
