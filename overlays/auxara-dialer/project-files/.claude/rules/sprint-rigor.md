...
- [ ] `npm run gates:all` passes
- [ ] Local verification matched the risk: relevant gates for docs-only work, `npm run verify` for implementation work, and the full local CI-equivalent bundle for sprint-close / integration-branch / backend-contract / release-candidate work.
- [ ] For any sprint-close, integration-branch, backend-contract, or release-candidate claim, local proof includes `npm run gate:audit`, `npm run verify`, `npm run test:integration`, and `docker build -t auxara-backend:ci .`; remote GitHub Actions is only required on `main`, `milestone/**`, `release/**`, or manual `workflow_dispatch`.
...
- [ ] **Auditor cadence run** (the fleet's audit lenses, not just the mechanical gates): **per slice**, run the domain/security auditor lenses that match *what the slice touched* — `cybersecurity-auditor` for anything touching auth/RBAC/tenant-isolation/secrets/telemetry/rate-limits/dependencies, `compliance-auditor` for anything that dials/texts/records/stores prospect data or mutates number/lifecycle state, `doctrine-drift-auditor` for anything governed by an ADR / decision-log row / ARC-006 authority tier / central registry (the **code-vs-our-own-doctrine** lens — it catches what the other auditors structurally can't: code that contradicts our settled doctrine, AND doctrine artifacts that contradict each other, e.g. the DNC registry-scrub hard-block vs ARC-006 Tier-1b "not per-call enforcement"), `ui-verifier` for **any slice with a visible frontend change** (the RENDERED-surface lens — computed styles/geometry at 390/768/1024/1440 + keyboard/reduced-motion + loading/empty/error states actually rendering + locked-mock fidelity; this is the lens the static `check:ui` gates + jsdom tests structurally can't provide, operationalizing the 2026-06-28 FIX-A rendered-verification requirement — the invisible-at-tablet toggle that shipped green), `performance-auditor` for **hot-path slices** (dialer-engine next-lead fetch, `call_events` projection writes, wallboard fanout, lead-import per-row loops, conversations-inbox reads, the per-dial compliance gate, long-lived BullMQ workers, the CallProvider cockpit render path — N+1/unbounded-read/missing-index/re-render-storm/bundle-growth/worker-leak hazards), `functionality-parity-auditor` for **any slice that ships a backend endpoint/contract WITHOUT its frontend in the same slice** (the delivery-chain lens — decided→built→wired→called→reachable; its mechanical floor is `gate:endpoint-wiring`, whose `PENDING_WIRING` row is the slice's honest deferral, and the auditor re-triages every row — operationalizes the 2026-07-08 "backend built, never wired, nothing caught it" directive), and the relevant skill lens (e.g. `security-review-hardening`) otherwise. **After EVERY production-affecting merge**, run `release-verifier` — the merge loop is NOT closed without its verdict (CI green ≠ deployed product works; it verifies Railway deploy status + `/api/ready` + frontend reachability + a Sentry new-error sweep, operationalizing the 2026-06-19 broken-prod-that-passed-every-gate incident). **At sprint/phase close**, run a **whole-app domain + security + doctrine + performance + parity + journey sweep** — a full-surface `cybersecurity-auditor` + `compliance-auditor` + `doctrine-drift-auditor` + `performance-auditor` + `functionality-parity-auditor` (whole-app delivery-chain walk + PENDING_WIRING re-triage — the built-never-wired class lives BETWEEN slices, so only the whole-app pass sees it) + `user-journey-auditor` (the ICP's-seat walk — per-persona jobs incl. day-zero signup→first-dial, workflow loopholes, improvement candidates as decision inputs; operationalizes §2a's workflow-loophole audit — the 2026-06-19 user-found-it incident is the class this catches) pass over the merged result, not only the per-slice diffs — so cross-slice drift, newly-connected surfaces, and accumulated scale hazards are caught; the sweep also **re-triages every OPEN `docs/BUG_BACKLOG.md` row** (still valid? still deferred-with-reason? now exploitable/fixable → pull it in). Findings are remediated or filed to `docs/BUG_BACKLOG.md` with a tier (exploitable-now → fix this turn; deferred → backlog row + reason). A green `gates:all` is necessary but NOT sufficient — the auditor lenses catch what the mechanical gates can't.
...
- A **scrapped or dropped** decision (see decision-log §"⛔ Scrapped & Dropped" — e.g. parallel/predictive dialing, AMD, the per-number dial cap, the tentative-hold layer) is being reintroduced into a plan, sprint, schema, prompt, or code — STOP. Re-expanding killed scope is a **fresh explicit user decision**, never a latent assumption baked back in (ADR-DLR-001 reversal discipline). Deferred ≠ dropped: a parked future feature is fine; a *killed* one is not.
...
**L1 — Implementer hangs cluster at the heavy/DB test phase → fast-check-only implementer; the orchestrator owns the DB suite (2026-06-14, S1.2-C2).** Two implementer agents (C1, C2) died/hung late in their runs, both around writing or running the DB-backed test suite. Root factors: (a) the verbose `test:integration` log floods an already-large agent context at the very end of a long run; (b) the one shared local `auxara-testdb` (port 55432) means two concurrent runners hit phantom "DB unreachable" failures the agent can't distinguish from real ones and thrashes on; (c) lost-work/redo from worktree↔cwd races lengthens the run. **Prevention (wired):** the sprint-implementer contract now runs ONLY fast bounded checks (`typecheck` / `lint` / `format:check` / `gates:all`) and **writes** but does **not run** `test:integration` or the full `verify`; the orchestrator runs the DB suite **serialized as the sole local test-DB user** + adversarial review before merge. Fast typecheck/gates still catch type and gate errors early; runtime/DB behavior is the orchestrator's gate. Codified in `.claude/agents/sprint-implementer.md` + `docs/agent-prompts/orchestrator-handoff-context.md` §2. (The "one test-DB runner at a time" rule is the same lesson from the operator side — never run two `test:integration` against the shared DB concurrently.)

**L2 — Remote CI is milestone infrastructure, not the ordinary pre-customer PR blocker (2026-07-11, S1.3 integration closure).** GitHub Actions can fail before any repo step runs because of external runner/account/platform state; that is a hosting signal, not a code verdict. In S1.3 closure, the local CI-equivalent bundle (`gate:audit`, `verify`, `test:integration`, Docker build) produced better code evidence than the zero-step remote failure and caught a real Redis TTL test-isolation issue that was fixed locally. **Prevention (wired):** ordinary PRs attach local CI-equivalent proof; remote Actions runs only on `main`, `milestone/**`, `release/**`, or manual `workflow_dispatch`, and is required again for customer-impacting milestones/releases. Do not let blocked remote Actions falsely stop a locally-proven ordinary PR; do not let "local-first" become "skip proof."
