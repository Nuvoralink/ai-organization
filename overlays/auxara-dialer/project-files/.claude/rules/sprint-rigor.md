...
- [ ] For any sprint-close, integration-branch, backend-contract, or release-candidate claim, `npm run ci` completed with a real exit 0 and exercised `gate:audit`, `verify`, `test:integration`, and `docker build -t auxara-backend:ci .`; no remote status check substitutes for this local proof.
...
**L2 — Local CI is the sole merge gate (2026-07-31).** GitHub-hosted CI was retired because its paid runner repeatedly refused jobs before any repository step ran while the complete local aggregate already exercised the authoritative lanes. **Prevention (wired):** `gate:local-ci-contract` proves the aggregate retains `gate:audit`, `verify`, `test:integration`, and Docker build; every PR attaches the exact `npm run ci` result. Do not treat a GitHub status as proof, and do not let "local-only" become "skip proof."
