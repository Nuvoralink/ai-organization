...
- [ ] **Auditor cadence is staged by functional acceptance.** Matching auditors may run in parallel during root-cause, implementation, deploy-safety, and deployed-proof stages, but their ordinary findings are queue-only and MUST NOT redirect implementation until the original deployed journey is accepted. Only the interruption classes in `delivery-lifecycle.v1.json` may stop that loop. `release-verifier` runs immediately after every production-affecting merge and makes the original core-flow result the functional-acceptance authority; CI/readiness alone cannot pass it. After acceptance, remediate queued findings and run the applicable cybersecurity, compliance, doctrine, performance, parity, UI, and journey lenses. At sprint/phase close, run the whole-app sweep and re-triage every OPEN `docs/BUG_BACKLOG.md` row. A green mechanical gate is neither functional proof nor sufficient broad assurance.
