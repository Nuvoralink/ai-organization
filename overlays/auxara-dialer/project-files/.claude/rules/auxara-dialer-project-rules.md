...
- Saved calls, dispositions, recordings, and audit rows stay **readable for as long as they're retained** (a mapper change must not break an older persisted payload) — but retention is **not "forever."** **Retention is the tenant's obligation, not ours:** we are the data *processor*, and the tenant (the liable caller — ADR-CMP-001/ARC-006) is the *controller*. We hold tenant data while the workspace is active so it can operate and export it. At paid-period closure the approved policy enters one **30-day read-only export/reactivation window**, then a controlled purge deletes the tenant's data, including its internal-DNC and compliance evidence; only the PII-free business record and purge receipt remain. A global user identity survives only while another membership still requires it. The tenant's own legal-retention duty is met by export, not a hidden platform archive. This irreversible policy remains activation-gated on the named counsel, backup-horizon, and provider-retention checks in REC-002 and `docs/app-plan/product/tenant-offboarding-brief.md`.
