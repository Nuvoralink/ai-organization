...
- Functional work follows `.claude/rules/functionality-first-delivery.md`: root cause/current contract → targeted biting proof → required pre-merge audits and blocker classification → deploy safety → deployed functional proof → hardening/broad assurance. Functionality-first changes remediation order, never auditor cadence: adversarial review plus every applicable specialist lens is required before merge. Fix BLOCK findings now; only verified bounded fail-safe FIX-NEXT residuals outside every blocker class may wait, with a durable backlog row before merge. Merge preparation remains concurrent: proven documentation/file-map/non-functional projection drift gets an active parallel repair owner and does not pause implementation, focused proof, auditors, or real-blocker repair; final merge still requires every mandatory gate green. Provider-facing work must carry current official-documentation and installed-SDK evidence. **During implementation/review repair**, run focused biting tests plus `npm run proof:changed`; do not repeatedly run `verify`, `gates:all`, `test:integration`, or `ci`. With explicit human deployment authority, a deploy-safe, audited, zero-BLOCK candidate may merge/deploy for observed product proof before broad hardening. After deployed functional proof is accepted, remediate queued findings and run **one `npm run ci` — the COMPLETE sprint/release closure gate** (`gate:audit` → `verify:merge` → `test:integration` → `docker build`) on the final hardened candidate. GitHub-hosted CI remains retired; a status is not proof. The shared Postgres/Redis pair remains lease-protected; only the orchestrator may recover it after proving no DB/test process is active.
