...
- Functional work follows `.claude/rules/functionality-first-delivery.md`: root cause/current contract → targeted biting proof → deploy safety → deployed functional proof → hardening/broad assurance. Auditors may run before functional acceptance, but their ordinary findings are queue-only until the original deployed journey works; only migration/schema apply failure, database-integrity or irreversible-data-loss risk, build/startup failure, Railway deploy/readiness failure, or catastrophic irreversible security risk interrupts that loop. Provider-facing work must carry current official-documentation and installed-SDK evidence. **During implementation/review repair**, run focused biting tests plus `npm run proof:changed`; do not repeatedly run `verify`, `gates:all`, `test:integration`, or `ci`. With explicit human deployment authority, a deploy-safe functional candidate may merge/deploy for observed product proof before broad hardening. After deployed functional proof is accepted, remediate queued findings and run **one `npm run ci` — the COMPLETE sprint/release closure gate** (`gate:audit` → `verify:merge` → `test:integration` → `docker build`) on the final hardened candidate. GitHub-hosted CI remains retired; a status is not proof. The shared Postgres/Redis pair remains lease-protected; only the orchestrator may recover it after proving no DB/test process is active.
