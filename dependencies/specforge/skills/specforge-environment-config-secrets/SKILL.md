---
name: specforge-environment-config-secrets
description: Create environment, configuration, secret handling, access, local/staging/production isolation, feature flag, and leak-response documentation.
---

# Environment, Config, and Secrets Documentation
...
```text
Research current official environment, secret-management, configuration, feature-flag, credential rotation, deployment platform, cloud provider, and CI/CD guidance for [stack/cloud/CI/deployment]. Prefer official vendor docs, OWASP, NIST SSDF, and CISA sources. Record source version/date and requirement IDs affected.
```
...
- Existing repo evidence: env examples, config files, CI, deployment config, secret references

## Output files

Create or update:

- `docs/app-plan/engineering/22-environment-config-secrets.md`
...
- If a repo contains secrets, document the risk and recommended response without repeating the values.
- Config required for auth, payments, production deployment, email, storage, analytics, and AI providers must be treated as high risk.
