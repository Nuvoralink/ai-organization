...
2. A machine-local root registry in the primary Git checkout resolves user-home and project tokens without committing machine-specific paths. Linked worktrees reuse that single binding through Git's common directory, while a deliberately present worktree-local registry is an explicit override.
