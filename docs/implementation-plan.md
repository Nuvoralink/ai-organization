...
| COORDINATION-RUNNER-DELIVERY-001 | OPEN | CoachAI runs coordination OBSERVE but has no `agent:run` bounded runner: the engine and lifecycle-hook registration path were delivered, while `scripts/run-bounded-agent.mjs` + its `lib/boundedProcess.mjs` seam + the `agent:run` package script remain dialer-only, so CoachAI claim registration works only through lifecycle task contracts and every bounded local dispatch there is unbounded AND unclaimed. | Template the bounded runner (runner + boundedProcess seam + package script + its biting tests) into the bootstrap skill and both project overlays as a managed mapping, mirroring the dialer file as canonical; deliver via scoped install; CoachAI AGENTS.md then drops its interim only-lifecycle-path caveat. | The bootstrap template already ships the boundedProcess seam (2026-07-28); the runner mapping is the missing half. | Positive: a CoachAI `npm run agent:run -- --brief` dispatch registers a claim and tree-kills on timeout. Negative/killer: remove the runner mapping from an overlay and the fleet/organization gate must flag the missing managed artifact, not silently skip. |
