...
| COORDINATION-RUNNER-DELIVERY-001 | RESOLVED — 2026-07-28; validated by `tests/bootstrap-templates.test.mjs` (`COORDINATION-RUNNER-DELIVERY-001` template/overlay byte-parity mutation), `tests/project-overlay.test.mjs` (`COORDINATION-RUNNER-DELIVERY-001` missing-mapping mutation), and the scoped CoachAI exit-0/exit-124/OBSERVE active→retired probes | CoachAI runs coordination OBSERVE but had no `agent:run` bounded runner: the engine and lifecycle-hook registration path were delivered, while `scripts/run-bounded-agent.mjs` + its `lib/boundedProcess.mjs` seam + the `agent:run` package script remained dialer-only, so CoachAI claim registration worked only through lifecycle task contracts and every bounded local dispatch there was unbounded AND unclaimed. The Dialer-proven runner also consumes `scripts/lib/dispatchBoundary.mjs`; the resolved delivery manages that load-bearing dependency instead of installing a broken two-file subset. | The bounded runner, deliberate full `boundedProcess` union, shared dispatch-boundary parser, package script, and biting tests are templated in the bootstrap skill and mapped in both project overlays; CoachAI received the scoped install and its AGENTS.md now names both honest claim paths. | The bootstrap template's earlier liveness-only seam was promoted to the Dialer-proven whole-tree bounded-process superset, retaining `isProcessAlive` for lifecycle consumers and adding `runBounded` for the runner without a parallel process policy. | Positive: a CoachAI `npm run agent:run -- --brief` dispatch registered an active OBSERVE claim, released it to `retired`, and tree-killed an over-deadline child with exit 124. Negative/killer: removing `coachai-project-bounded-runner` from an overlay fixture leaves the managed artifact absent and the installed organization parity gate names `scripts/run-bounded-agent.mjs` instead of silently skipping it. |
